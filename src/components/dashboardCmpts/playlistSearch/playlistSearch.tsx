...
export default function PlaylistSearch({ autoWidth, inputSearchHeading }: PlaylistSearch_Interface) {
  ...
  const getPlaylist = async (event: React.FormEvent) => {
    ...
    try {
      ...
      while (allFilteredPlaylists.length < targetMatchCount && searchAttempts < maxSearchAttempts) {
        searchAttempts++
        ...
        for (let i = 0; i < playlists.length && allFilteredPlaylists.length < targetMatchCount; i += batchSize) {
          ...
          const batchPromises = batch.map(playlist => fetchPlaylistDetails(playlist, access_token))
